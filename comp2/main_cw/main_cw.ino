...
const int followLineTime4 = 7000;
...
const int wallDistanceTolerance3 = 5; // tolerannce for usonnic sensor readings

// block distannces and times
const int blockDistance = 10;// if fronntDist smaller than this, we go forwrad a bit and pick up the block
...
const int blockDistance2 = 150; // dist from front to wall at first right junnctio
const int blockDistance3 = 70; // dist from front to wall at seconnd right.
...
void loop() {
  switch (robotState) {
    ...
    case 2: // go through tunnel
        ...
        if ((sensorReading == "0100" || sensorReading == "0010" || sensorReading == "0110" || !detectRHSBlock()) && (currentTime - startTime) > tunnelTime) { // reached end of tunnel
            stop();
            sweepForLine(true);
            startTime = millis();
            robotState = 3;
    ...
    case 3: // line following after tunnel - robot looks for block
        ...
        if (currentTime - startTime >= followLineTime4) {
              ...
              if (frontDist < 5 && frontDist != 0) { // robot found block inn fronnt
                  stop();
                  forward();
                  delay(blockTime);
                  robotState = 6;
              ...
              if (frontDist < blockDistance3 + wallDistanceTolerance3 && frontDist > blockDistance3 - wallDistanceTolerance3 && frontDist != 0) {
                stop();
                ...
                if (detectRHSBlock()) {
                  ...
                  robotState = 6;
                  break;
              } else if (frontDist < blockDistance2 + wallDistanceTolerance3 && frontDist > blockDistance2 - wallDistanceTolerance3 && frontDist != 0){
                                stop();
                          // detect block on RHS of robot with IR sensor
                if (detectRHSBlock()) {
                  RHSBlock = true;
              ...
              // ONNLY DO THIS IF HOLDINNG BLOCK = FALSE> ONCE ACTIVATED no NEED TO SCAN.
              } else {
                forward();
              }
              }
        ...
        if (leftDist < (leftWallDistance1 + wallDistanceTolerance2) && leftDist != 0 && (currentTime - startTime) >= followLineTime2) { // maybe time from block to ramp instead of tunnel to ramp
        ...
        if (holdingBlock || currentTime - startTime < followLineTime4) {
          checkRightTurn();
        }
    ...
    case 4: // on ramp
        ...
        if ((sensorReading == "0100" || sensorReading == "0010" || sensorReading == "0110") && (currentTime-startTime) >= rampTime) {
            leftCount = 0; // reset left juctionn counnt
        ...
        // if robot is close to fronnt wall, it has left the ramp. return to linne following. include this checkRightTurn 
/*         frontDist = frontDistance();
