...
const int farLeftLineSensor = 7; // 7 nnot working
...
unsigned long startTime2; // records time when we finnish the block collecting routine
...
/*   while (true) {
    Serial.println(checkFrontDistance(10));
  } */
...
/*         followLine();
        ...
        if (sensorReading == "0111" || currentTime-startTime >= 2000) {
...
/*         if (sensorReading == "0111" && previousSensorReading != "0111") { // onnly increase junction count for first instance
    ...
    case 2: // on ramp
        ...
        if ((sensorReading == "0100" || sensorReading == "0010" || sensorReading == "0110")) {
            foundLine = true;
        }
 ...
 /*        if ((currentTime - startTime) >= (rampTime * 3) && sweepCount == 1) {
        ...
        if (foundLine && (currentTime-startTime) >= rampTime) {
            detectedBlock = false;
        ...
        if (checkFrontDistance(frontWallDistance2) && (currentTime-startTime) >= rampTime) {
    ...
    case 3: // line following after ramp - robot looks for block
        ...
        // robot got past the block junction - we use this instead of leftCount because front usonic distance is not as reliable.
        if (sensorReading == "0111" || detectedBlock || (currentTime -startTime >= followLineTime3)) {
        ...
        if (!foundLine) {
          ...
          if (sensorReading == "0000" && detectLHSBlock() && pastBlockJunction) {
            // robot in tunnel
            robotState = 4;
            startTime = millis();
            forward();
    ...
    case 5: // line following after tunnel
        ...
        if (sensorReading == "0111") { 
            if (previousSensorReading == "0111") { // do nothing if this isn't first instance of detecting the junnctionn
                ...
                } else if (rightCount == 2) { // robot wasn't pushing the block. reset to state 1.
                    startTime = millis();
                    robotState = 1;
                    break;
                }
...
void followLine() {
  ...
  if (sensorReading == "0110") { // 1 is white, 0 is black
    ...
    switch (robotState) {
      ...
      case 3:
        if ((currentTime-startTime) < followLineTime2 || (holdingBlock && (currentTime-startTime2 < followLineTime3))) { // not reached tunnel yet
            sweepForLine(false);
        } else {
            return;
        }
